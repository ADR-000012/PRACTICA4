...
<?php 
   ...
   $correo= $_POST['correo']; // Capturamos ẹl corre electrónico
   $telefono = $_POST['telefono']; // Capturamos el teléfono 
   $fuente = $_POST['fuente']; // Capturamos la fuente de información 
   $temas_interes = isset($_POST['temas_interes']) ? implode(", ", $_POST['temas_interes'] ): "No se seleccionó ningin tema"; // Capturamos temas de interés 
   $opciones = isset($_POST['opciones']) ? implode(", ", $_POST['opciones']) : "No seleccionó ninguna opcin"; // Capturamos opciones de participación
   $comentarios = $_POST['comentarios']; // Capturamos comentar1os
   ...
   echo "<p><strong>0pciones de Participación:</strong> $opciones</p>"; 
